...
def start(feq=60, pid_path='frequency.pid'):
    ...
    try:
        ...
        print("Frequency Task Checker is started with pid {pid}".format(pid=pid))
        pid_file.close()
    except (IOError, TypeError):
        process = Process(target=frequency_checker, kwargs={'pid_path': pid_path, 'feq': feq})
        process.start()
        pid_file = open(pid_path, 'w')
        pid_file.write("{pid}".format(process.pid))
        pid_file.close()
        print("Start a Frequency Task Checker process with pid {pid}".format(pid=process.pid))
